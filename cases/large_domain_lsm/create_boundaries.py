...
def write_LBC(u, v, thl, qt, thls, itot, jtot, nprocx, nprocy, mpiidx, hour, minutes, iexpnr, output_dir):
    ...
    for mpiidy in yprocs:
        ...
        # Open binary file
        name = '{0:}/lbc{1:03.0f}h{2:02.0f}m_x{3:03d}y{4:03d}.{5:03d}'.format(
                output_dir, hour, minutes, mpiidx, mpiidy, iexpnr)
...
if __name__ == '__main__':
    ...
    # HARMONIE data
    # --------------------------
    date_hm = datetime.datetime(year=2016, month=8, day=15)
    ...
    # averaged over (+/-) DALES domain
    # -----------------
    T_mean  = T [t0:t1+1,::-1,:,:].values.mean(axis=(0,2,3))
    qv_mean = q [t0:t1+1,::-1,:,:].values.mean(axis=(0,2,3))
    ps_mean = ps[t0:t1+1,:,:     ].values.mean()

    ph_mean = grid_sig.calc_half_level_pressure(ps_mean)
    p_mean  = grid_sig.calc_full_level_pressure(ph_mean)

    nc_file = nc4.Dataset('{0}/backrad.inp.{1:03d}.nc'.format(output_dir, iexpnr), 'w')
    dims = nc_file.createDimension('lev', T_mean.size)
